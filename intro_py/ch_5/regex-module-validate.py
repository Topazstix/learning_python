...
def main() : 
    ...
    ## This is a loop statement allowing the user to input only numbers for calculation.
        flag = False
        while not flag:
            validate =  re.compile('\d+(\.\d+)?')
            radius=input("Enter radius of sphere in inches: ")
            if validate.match(radius) :
                radius= float(radius)
                sphereVolume(radius)
                sphereSurface(radius)
                flag = True
            elif validate.match(radius) == None :
    ...
    ## This is a loop statement allowing the user to input only numbers for calculation.
        ...
        while not flag:
            ...
            if validate.match(height) and validate.match(radius):
                radius = float(radius)
                height = float(height)
                coneVolume(radius, height)
                coneSurface(radius, height)
                flag = True
            elif validate.match(height) or validate.match(radius) == None:
                print("Incorrect, please enter a number.")
        break
